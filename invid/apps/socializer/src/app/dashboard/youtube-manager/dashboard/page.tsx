...
import { Upload, Youtube, BarChart3, Eye, Users, Edit2, X, Sparkles, Loader2, AlertCircle, Plus } from "lucide-react";
...
const DashboardPage = () => {
    ...
    useEffect(() => {
        ...
        if (!token) {
            router.push("/dashboard/youtube-manager");
            return;
        }
    ...
    const fetchChannelData = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (errorCode === 401) {
                    toast.error("Authentication failed. Please reconnect your YouTube account.");
                } else {
    ...
    const handleGenerateHints = async () => {
        ...
        try {
            const context = `Analyze this YouTube video and provide optimization suggestions in JSON format.
...
{
  "titles": ["suggestion1", "suggestion2", "suggestion3"],
  "descriptions": ["description1", "description2"],
  "tags": ["tag1, tag2, tag3", "alternative1, alternative2, alternative3"],
  "general": "Brief overall tips"
}

Requirements:
- Titles: Provide 3 SEO-optimized, engaging title variations (max 60 characters each)
- Descriptions: Provide 2 well-structured description options with keywords and CTAs
- Tags: Provide 2 sets of comma-separated relevant tags/hashtags
- General: Brief tips for improving engagement`;
            ...
            if (result.success) {
                try {
                    ...
                    setAiHints(parsedData);
                    toast.success("AI suggestions generated!", { id: toastId });
                } catch (parseError) {
                    console.error("Parse error:", parseError);
                    toast.error("Failed to parse AI suggestions", { id: toastId });
                }
            } else {
    ...
    const handleSaveMetadata = async () => {
        ...
        const videoId = editingVideo.snippet?.resourceId?.videoId || editingVideo.id;
        setSaving(true);
    ...
    return (
        <UIWrapper classname="min-h-screen bg-gradient-to-br from-gray-950 via-black to-black">
            ...
            <div className="flex-1 w-full min-w-7xl mx-5 px-4 pt-4 pb-8 overflow-y-auto">
                {/* Loading State */}
                {loading && !channelData && (
                        ...
                        <div className="flex flex-col items-center text-center space-y-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-white mb-2">
                                    {error.code === 401 ? 'Authentication Required' : 'Error Loading Data'}
                                </h3>
                        ...
                        <Suspense fallback={<div>Loading...</div>}>
                            ...
                            {analytics && (
                                <div className="glass rounded-2xl p-6 border border-purple-500/20 bg-gradient-to-br from-purple-500/5 to-transparent hover:border-purple-500/30 transition-colors duration-300">
                                    <h3 className="text-xl font-bold text-white mb-4">30-Day Analytics</h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="text-center">
                                            <div className="text-lg font-bold text-white">
                                                {Math.round((analytics.overview?.rows?.[0]?.[1] || 0) / 60)} hrs
                                            </div>
                        ...
                        <Suspense fallback={<div>Loading...</div>}>
                            ...
                            {channelData?.recentVideos && (
                                <div className="glass rounded-2xl p-6 border border-white/10 hover:border-cyan-500/30 transition-colors duration-300">
                                    <h3 className="text-xl font-bold text-white mb-4">All Videos ({channelData.recentVideos.length})</h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        {channelData.recentVideos.map((video: any, index: number) => (
                                            <div key={index} className="glass rounded-xl overflow-hidden hover:scale-105 hover:border-red-500/30 transition-all duration-300 group border border-white/10">
                                                ...
                                                <div className="p-4">
                                                    <h4 className="text-white font-medium line-clamp-2 mb-2 text-sm" title={video.snippet?.title}>
                                                        {video.snippet?.title}
                                                    </h4>
                                                    ...
                                                    <div className="flex items-center justify-between text-xs">
                                                        <span className={`px-2 py-1 rounded-full font-medium ${video.status?.privacyStatus === 'public' ? 'bg-green-500/20 text-green-300 border border-green-500/30' :
                                                            video.status?.privacyStatus === 'private' ? 'bg-red-500/20 text-red-300 border border-red-500/30' :
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-1.5 rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 hover:from-purple-500/30 hover:to-pink-500/30 text-purple-300 hover:text-purple-200 transition-all duration-200 border border-purple-500/20"
                                                            title="Edit metadata"
                                                        >
            ...
            {editingVideo && (
                <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-white/20">
                        <div className="sticky top-0 glass border-b border-white/10 p-6 flex items-center justify-between backdrop-blur-xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-400">Update your video details with AI assistance</p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    maxLength={5000}
                                />
                                <p className="text-xs text-gray-500 mt-1">{editDescription.length}/5000 characters</p>
                            </div>
                                    ...
                                    {aiHints.descriptions && aiHints.descriptions.length > 0 && (
                                        <div className="glass rounded-xl p-5 border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-blue-500/5">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <h3 className="text-lg font-bold text-white">Description Suggestions</h3>
                                            </div>
                                            <div className="space-y-2">
                                                {aiHints.descriptions.map((desc, index) => (
                                                    <div
                                                        key={index}
                                                        className="flex items-start gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors group"
                                                    >
                                    ...
                                    {aiHints.tags && aiHints.tags.length > 0 && (
                                        <div className="glass rounded-xl p-5 border border-pink-500/30 bg-gradient-to-br from-pink-500/10 to-red-500/5">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <h3 className="text-lg font-bold text-white">Tags Suggestions</h3>
                                            </div>
                                            <div className="space-y-2">
                                                {aiHints.tags.map((tagSet, index) => (
                                                    <div
                                                        key={index}
                                                        className="flex items-start gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors group"
                                                    >
                                                        <div className="flex-1">
                                                            <p className="text-sm text-gray-300 leading-relaxed">{tagSet}</p>
                                                        </div>
                                                        <button
                                                            onClick={() => {
                                                                setEditTags(tagSet);
                                    ...
                                    {aiHints.general && (
                                        <div className="glass rounded-xl p-5 border border-green-500/30 bg-gradient-to-br from-green-500/10 to-emerald-500/5">
                                            <div className="flex items-center gap-2 mb-3">
                                                ...
                                                <h3 className="text-lg font-bold text-white">General Tips</h3>
                                            </div>
                                            <p className="text-sm text-gray-300 leading-relaxed">{aiHints.general}</p>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 glass rounded-xl border border-white/10">
                                <div className="text-center">
                                    <div className="text-lg font-bold text-white">
                                        {editingVideo.statistics?.viewCount ? parseInt(editingVideo.statistics.viewCount).toLocaleString() : '0'}
                                    </div>
                                    <div className="text-xs text-gray-400">Views</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-lg font-bold text-white">
                                        {editingVideo.statistics?.likeCount ? parseInt(editingVideo.statistics.likeCount).toLocaleString() : '0'}
                                    </div>
                                    <div className="text-xs text-gray-400">Likes</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-lg font-bold text-white">
                                        {editingVideo.statistics?.commentCount ? parseInt(editingVideo.statistics.commentCount).toLocaleString() : '0'}
                                    </div>
